...
export const EXPANDING_IDEA_INPUT_MAP: Record<
  EEIQuestionKeys,
  ConceptIgnitionInput
> = {
  DESCRIBE: {
    question: 'Describe your idea in one sentence',
    placeholder:
      'I want to leverage our network of retail stores to deliver healthcare services',
    rows: 2,
    fieldType: 'textarea',
    required: true,
  },
  PROBLEM: {
    ...
    placeholder:
      'Busy customers lack healthcare access and the time to juggle their to-do list.',
    rows: 2,
    fieldType: 'textarea',
  },
  CUSTOMER: {
    ...
    placeholder:
      'Time-strapped consumers in suburban areas who need to maximize time.',
    rows: 2,
    fieldType: 'textarea',
  },
  SUCCESS: {
...
export const NEW_OPPORTUNITY_INPUT_MAP: Record<
  INOQuestionKeys,
  ConceptIgnitionInput
> = {
  TARGET: {
  ...
  PROBLEM: {
    ...
    placeholder:
      'Customers struggle to be assigned a family doctor and access care quickly',
    rows: 2,
    fieldType: 'textarea',
  },
  INTEREST: {
    ...
    placeholder:
      'We have broad reach across the country and have experience training workforces',
    rows: 3,
    fieldType: 'textarea',
  },
  SUCCESS: {
    question: 'What will success look like?',
    placeholder:
      'Create a new revenue stream and increase customer satisfaction by 10%',
    rows: 2,
    fieldType: 'textarea',
  },
};
...
type RenderIgnitionInput = <T extends ConceptSeedType = ConceptSeedType>(
  ...
  setter: React.Dispatch<
    React.SetStateAction<IConceptSeedAttribute<keyof ConceptSeedTypeMap[T]>[]>
  >,
) => JSX.Element | null;
...
const IgniteConcept: FunctionComponent = () => {
  ...
  const [expandingIdea, setExpandingIdea] = useState<
    IConceptSeedAttribute<EEIQuestionKeys>[]
  >(
    // Create deep copy.
    JSON.parse(JSON.stringify(INITIAL_EXPANDING_IDEA)),
  );
  const [newOpportunities, setNewOpportunities] = useState<
    IConceptSeedAttribute<INOQuestionKeys>[]
  >(
    // Create deep copy.
  ...
  const renderIgnitionInput: RenderIgnitionInput = useCallback(
    (type, item, index, setter) => {
      if (type === 'UNKNOWN') return null; // Add this line to prevent rendering of unknown seed type
      const inputProps: ConceptIgnitionInput = CONCEPT_SEED_TYPE_QUESTIONS[
        type
      ][item.question] as ConceptIgnitionInput;
      const sharedProps = {
        ...
        onChange: (
          ...
          setter((prev) =>
            prev.map((prevItem, prevIndex) =>
              prevIndex === index
                ? { ...prevItem, answer: event.target.value }
                : prevItem,
            ),
          );
        },
      ...
      return (
        <div
          className='flex gap-3.5 self-stretch'
          key={sharedProps.name + index}
        >
          ...
          <section className='flex w-full flex-row flex-wrap justify-center gap-6'>
            <Card.Ignition
              header={{
                ...
                description:
                  "Describe an idea you already have and we'll help you to validate it.",
                color: 'bg-violet-100',
                image: {
                ...
                <>
                  <button
                    ...
                    disabled={expandingIdea.some(
                      (item) =>
                        !item.answer &&
                        !!EXPANDING_IDEA_INPUT_MAP[item.question].required,
                    )}
                    onClick={() =>
                      handleIgnition(
                        expandingIdea,
                ...
                <>
                  <button
                    ...
                    disabled={newOpportunities.some(
                      (item) =>
                        !item.answer &&
                        !!NEW_OPPORTUNITY_INPUT_MAP[item.question].required,
                    )}
                    onClick={() =>
                      handleIgnition(
                        newOpportunities,
