...
import { IBulkConceptUpdate, IPropertyDefinition } from '@libs/api/types';
import { cn } from '@libs/utils/react';
import { extractOptionValues } from '@libs/utils/propertyColors';
...
const CONCEPT_STATUS_OPTIONS: { value: string; label: string }[] = [
  { value: 'new', label: 'New' },
  { value: 'ideating', label: 'Ideating' },
  { value: 'in_review', label: 'In Review' },
  { value: 'prototyping', label: 'Prototyping' },
  { value: 'proof_of_concept', label: 'Proof of Concept' },
  { value: 'minimum_viable_product', label: 'Minimum Viable Product' },
...
interface BulkEditConceptsModalProps {
  isOpen: boolean;
  onClose: () => void;
...
const BulkEditConceptsModal: React.FC<BulkEditConceptsModalProps> = ({
  isOpen,
  onClose,
  ...
  const bulkUpdate = useBulkConceptUpdate({ onRescoreStarted });
  ...
  const [customPropertyValues, setCustomPropertyValues] = useState<
    Record<string, any>
  >({});
  const [changedPropertyKeys, setChangedPropertyKeys] = useState<Set<string>>(
    new Set(),
  );
  ...
  const activeProperties = useMemo(
    () =>
      (propertyDefinitions ?? [])
        .filter((p: IPropertyDefinition) => p.isActive)
        .sort(
          (a: IPropertyDefinition, b: IPropertyDefinition) =>
            a.displayOrder - b.displayOrder,
        ),
    [propertyDefinitions],
  );

  const count = selectedConceptUuids.length;
  const hasChanges =
    status !== '' || scoringConfigUuid !== '' || changedPropertyKeys.size > 0;
  ...
  const handlePropertyChange = (key: string, value: any) => {
    setCustomPropertyValues((prev) => ({ ...prev, [key]: value }));
    setChangedPropertyKeys((prev) => new Set(prev).add(key));
  };

  const handlePropertyReset = (key: string) => {
    setCustomPropertyValues((prev) => {
      const next = { ...prev };
      delete next[key];
      return next;
    });
    setChangedPropertyKeys((prev) => {
  ...
  const resetForm = useCallback(() => {
    ...
    setChangedPropertyKeys(new Set());
  }, []);

  const handleApply = () => {
    const payload: IBulkConceptUpdate = {
      conceptUuids: selectedConceptUuids,
    };

    if (status) payload.status = status;
    if (scoringConfigUuid) payload.scoringConfigUuid = scoringConfigUuid;

    if (changedPropertyKeys.size > 0) {
      const customProperties: Record<string, unknown> = {};
      changedPropertyKeys.forEach((key) => {
        customProperties[key] = customPropertyValues[key];
      });
  ...
  const renderPropertyField = (prop: IPropertyDefinition) => {
    const key = prop.key;
    const hasChanged = changedPropertyKeys.has(key);
    ...
    switch (prop.propertyType) {
      case 'text':
        return (
          <FormTextInput
            ...
            value={customPropertyValues[key] ?? ''}
            onChange={(v) => handlePropertyChange(key, v)}
            onReset={() => handlePropertyReset(key)}
      ...
      case 'number':
        return (
          <FormNumberInput
            ...
            value={customPropertyValues[key] ?? ''}
            onChange={(v) =>
              handlePropertyChange(key, v === '' ? null : Number(v))
            }
            onReset={() => handlePropertyReset(key)}
            hasChanged={hasChanged}
            min={prop.config.min}
      ...
      case 'select': {
        const options = extractOptionValues(prop.config.options);
        return (
          <FormSelect
            key={prop.uuid}
            label={prop.name}
      ...
      case 'multi_select': {
        ...
        return (
          <FormMultiSelect
            ...
            placeholder='No change'
            options={options}
            selectedValues={customPropertyValues[key] ?? []}
            onChange={(values) => {
              if (values.length === 0 && !hasChanged) {
                handlePropertyReset(key);
              } else {
          ...
          <FormCheckbox
            ...
            onChange={(v) => {
              if (v === 'no_change') {
                ...
                handlePropertyChange(key, v === 'true');
              }
            }}
          />
        );
                  ...
                  <div className='min-h-0 flex-1 overflow-y-auto'>
                    <div className='space-y-5 px-6 py-5'>
                      <p className='aucctus-text-sm aucctus-text-secondary'>
                        Editing {count} concept{count !== 1 ? 's' : ''}. Only
                      ...
                      {activeProperties.length > 0 && (
                        <>
                          <div className='aucctus-border-secondary border-t pt-4'>
                            <p className='aucctus-text-xs-medium aucctus-text-tertiary uppercase tracking-wider'>
                              Custom Properties
                            </p>
                          </div>
                          {activeProperties.map(renderPropertyField)}
                        </>
                      )}
                    </div>
                  </div>
                  ...
                  <div className='flex shrink-0 items-center justify-end gap-3 border-t px-6 py-4'>
                    ...
                    >
                      {bulkUpdate.isLoading
                        ? 'Applying...'
                        : `Apply to ${count} concept${count !== 1 ? 's' : ''}`}
                    </button>
                  </div>
                </div>
              </div>
...
export default BulkEditConceptsModal;
