...
const SimpleList: FunctionComponent<SimpleListProps> = ({
  ...
  const numberOfItemsToAdd = useMemo(
    () => (items.length < minLength ? minLength - items.length : 0),
    [items.length, minLength]
  );
  const numberOfItemsToRemove = useMemo(
    () => (items.length > maxLength ? items.length - maxLength : 0),
  ...
  return (
    <div className={styles.simpleList}>
      ...
      <div className={styles.content}>
        {data.map((item) => (
          <span className={styles.item}>{isLoading ? <Loading /> : item}</span>
        ))}
      </div>
    </div>
  );
