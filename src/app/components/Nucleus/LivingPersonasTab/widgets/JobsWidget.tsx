/**
 * JobsWidget - Jobs to be Done list with priority scale indicator
 *
 * Displays persona jobs sorted by priority with a vertical gradient scale.
 * Matches CustomerJobs design from CustomerProfile:
 * - PriorityIndicator component for left-side scale
 * - orangeDark color scheme
 * - Briefcase icon per item
...
import { Briefcase, Pencil, Plus, X as XIcon } from 'lucide-react';
...
/** Props for the JobsWidget component */
...
const ICON_COLOR = 'stroke-orangeDark-900';
const ICON_BG = 'bg-orangeDark-100';

/**
 * JobsWidget Component
 */
const JobsWidget: React.FC<JobsWidgetProps> = ({
  ...
  return (
    ...
    >
      <p className='aucctus-text-secondary aucctus-text-sm mb-4'>
        Critical tasks the customer is trying to accomplish that our product can
        help with
      </p>
        ...
        {sortedItems.length === 0 && !isAdding && onAdd && (
          ...
          >
            ...
            >
              ...
              <span className='aucctus-text-sm'>Add a job</span>
            </button>
          </motion.div>
        )}
                ...
                <AnimatePresence>
                  {sortedItems.map((item, index) => (
                    ...
                    >
                      ...
                      >
                        <Briefcase className={cn('h-3 w-3', ICON_COLOR)} />
                      </div>
