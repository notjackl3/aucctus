/**
 * BehavioursWidget - Behaviours list (no scale bar, no priority)
 *
 * Displays persona behaviours preserving backend ordering.
 * Simplest widget — no vertical scale bar, no priority sort.
 * - Zap icon per item in blue circle
...
/** Props for the BehavioursWidget component */
export interface BehavioursWidgetProps {
  /** Widget title */
  title?: string;
  /** Icon variant */
  icon?: string;
...
/**
 * BehavioursWidget Component
 */
const BehavioursWidget: React.FC<BehavioursWidgetProps> = ({
  title = 'Behaviours',
  icon = 'zap',
  items,
  size = 'small',
  ...
  return (
    <GlassWidget
      ...
      iconColorClass='stroke-blue-600'
      size={size}
      showAddButton={!!onAdd}
    ...
    >
      <p className='aucctus-text-secondary aucctus-text-sm mb-4'>
        How this persona typically acts and interacts in relevant contexts
      </p>
...
export default BehavioursWidget;
