...
import { DEFAULT_SCORING_CATEGORIES } from './fixtures';
...
const ConceptScoringConfig: React.FC<ConceptScoringConfigProps> = ({
  ...
  useEffect(() => {
    ...
    if (!isInitialized) {
      if (apiCategoryCount > 0) {
        ...
        // API returned empty - show defaults (auto-init happens via useAutoInitScoringConfig)
        // Set both categories AND savedCategories to defaults so we don't show
        // "unsaved changes" banner on fresh database
        setCategories(DEFAULT_SCORING_CATEGORIES);
        setSavedCategories(DEFAULT_SCORING_CATEGORIES);
        setSelectedCategoryId(DEFAULT_SCORING_CATEGORIES[0]?.id || '');
      }
  ...
  const handleDiscardChanges = useCallback(() => {
    // Restore to last saved state (or defaults if nothing saved yet)
